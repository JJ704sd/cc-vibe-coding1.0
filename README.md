...
- 后台 CRUD 和上传接口缺少统一服务端鉴权保护。
- `/uploads/*` 静态文件挂载会绕过“仅发布内容可访问”的公开文件检查。
...
本次提交没有修改 API 代码。API 验证记录请参考 review 文档；继续开发或部署前仍应重新运行相关命令。
...
注意：`/api/public/uploads/:fileId` 的设计意图是只返回发布内容可达文件；但当前还存在 `/uploads/*` 静态挂载绕过风险，详见 review 文档。
...
1. 给后台 CRUD 和上传接口补服务端 admin session 鉴权。
2. 取消或限制 `/uploads/*` 静态挂载，统一公开文件访问路径。
3. 修正 `main.ts` 到 `buildServer` 的生产配置传递。
4. 统一 `STORAGE_DIR` 与 `UPLOAD_ROOT` 的语义。
5. 修正公开项目封面 URL 映射。
6. 为 route location 替换增加事务保护。
7. 继续补后台表单、排序、发布流程和 Web chunk 拆分优化。
...
不要只因为测试通过就宣称项目完成。这个项目还有明确的服务端鉴权、上传访问控制和生产配置问题需要关闭。

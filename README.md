...
- API tests: 6 files passed, 36 tests passed.
- API build: passed.
...
1. 取消或限制 `/uploads/*` 静态挂载，统一公开文件访问路径。
2. 修正 `main.ts` 到 `buildServer` 的生产配置传递。
3. 统一 `STORAGE_DIR` 与 `UPLOAD_ROOT` 的语义。
4. 修正公开项目封面 URL 映射。
5. 为 route location 替换增加事务保护。
6. 继续补后台表单、排序、发布流程和 Web chunk 拆分优化。
...
不要只因为测试通过就宣称项目完成。这个项目还有明确的上传访问控制、生产配置和内容发布链路问题需要关闭。

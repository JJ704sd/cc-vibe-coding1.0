...
这不是一个完全交付状态的产品。它更准确的状态是：核心结构、主要页面、基础数据链路、安全底座的一部分已经完成，但上传公开访问、生产配置传递、封面 URL 语义、内容发布闭环还需要继续收口。
...
后端仍通过 `fastifyStatic` 直接挂载存储目录。这个路径可能绕过“文件必须属于已发布内容”的检查。下一步应移除或限制该静态路径。
...
这些材料说明项目已经具备部署方向，但正式部署前仍要先关闭上传公开访问和生产配置传递问题。
...
在后台 API 鉴权合并前的隔离分支上，验证结果是：

- API tests：6 个测试文件通过，36 个测试通过。
- API build：通过。
...
3. 不要把 `/uploads/*` 当作最终公开图片方案。最终公开读取应走 `/api/public/uploads/:fileId`。
4. 不要把 `spin360` 和 `gallery` 合并成一个查看器。当前设计明确保留两类媒体体验。
5. 不要新增新的顶层实体来解决当前问题。当前核心模型应保持 `Project -> Location / MediaSet / Route -> MediaImage`。
6. 不要只因为测试通过就认为项目可以上线。上线前还必须处理公开文件、生产配置和上传目录语义。

## 推荐下一步

按风险优先顺序继续：

1. 移除或限制 `/uploads/*` 静态挂载，公开文件统一走 `/api/public/uploads/:fileId`。
2. 补测试证明孤立上传文件不能被公开访问，已发布内容引用文件可以访问。
3. 修正 `main.ts` 到 `buildServer` 的生产配置传递。
4. 统一 `STORAGE_DIR` 与 `UPLOAD_ROOT`，同步 API、env 示例、备份脚本和运维文档。
5. 修正公开项目和媒体组封面 URL 语义。
6. 给路线地点替换加事务。
7. 再继续打磨后台发布流程和前台空间叙事体验。
...
继续实现时，建议从“移除或限制 `/uploads/*` 静态挂载”开始，而不是继续扩展视觉功能。

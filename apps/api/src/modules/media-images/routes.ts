...
export function registerMediaImageRoutes(server: FastifyInstance, service: MediaImageService) {
  // GET /media-images
  server.get('/api/media-images', async (request) => {
    const { media_set_id } = request.query as { media_set_id?: string };
    return service.findAll(media_set_id);
  });

  // GET /media-images/:id
  server.get<{ Params: { id: string } }>('/api/media-images/:id', async (request, reply) => {
  ...
  server.delete<{ Params: { id: string } }>('/api/media-images/:id', async (request, reply) => {
    try {

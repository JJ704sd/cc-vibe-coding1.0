...
export function GalleryViewPage() {
  ...
  if (loading) {
    return (
      <div
        className="glass"
        data-testid="gallery-view-loading"
        style={{
          padding: '64px 40px',
          margin: '40px auto',
          maxWidth: '720px',
      ...
      >
        <SkeletonStack count={3} gap={14}>
          ...
          <Skeleton variant="rect" height={360} radius={20} aria-label="加载图集" />
        </SkeletonStack>
        <div className="muted" style={{ marginTop: '18px' }}>正在加载图集…</div>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div style={{ padding: '40px 24px' }}>
        <ErrorState
          testId="gallery-view-error"
          title="暂无可展示的图集媒体组"
          message={error?.message ?? '请先在后台为已发布项目创建 gallery 类型媒体组，并补充图片。'}
          cta={
